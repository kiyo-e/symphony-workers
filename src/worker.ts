...
import { configureWorkflow, loadWorkflow } from "./workflow";
...
export function createWorker(options: CreateWorkerOptions): ExportedHandler<Env> {
  configureWorkflow(options.workflowText);
  const app = createApp();
